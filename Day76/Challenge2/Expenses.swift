...
class Expenses: ObservableObject {
    ...
    var busineseItem: [ExpenseItem] {
        items.filter { $0.type == "Businese"}
    }
    ...
    init() {
        ...
        if let savedItems = UserDefaults.standard.data(forKey: "Items") {
            ...
            if let decodedItems = try? JSONDecoder().decode([ExpenseItem].self, from: savedItems) {
                // Ask the decoder tedso convert the data we received from UserDefaults into an array of ExpenseItem objects.
                items = decodedItems
